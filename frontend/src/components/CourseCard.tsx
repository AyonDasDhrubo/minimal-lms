    ...
    <div className="rounded-2xl shadow p-4 border flex flex-col gap-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Link href={`/admin/courses/${course._id}`} className="px-3 py-1 rounded-lg border hover:bg-gray-50">Manage</Link>
          {onDelete && <button onClick={() => onDelete(course._id)} className="px-3 py-1 rounded-lg border text-red-600 hover:bg-red-50">Delete</button>}
        </div>
      </div>
    </div>
  );
